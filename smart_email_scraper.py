#!/usr/bin/env python3
"""
Enhanced Email Scraper for ISEF Winners
- Deduces university from scholarship awards
- Guesses university email formats
- Searches Google/LinkedIn for emails
"""
...
UNIVERSITY_EMAIL_FORMATS = {
    ...
    'University of Arizona': ['{first}{last}@arizona.edu', '{first}.{last}@email.arizona.edu'],
    ...
    'UC Berkeley': ['{first}{last}@berkeley.edu', '{first}_{last}@berkeley.edu'],
...
def verify_email_guess(email, person_name):
    """Try to verify if an email exists by searching for it online"""
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
    }

    # Search for the email online
    query = f'"{email}"'
    ...
    try:
        ...
        if response.status_code == 200:
            # If we find the email in search results, it's likely valid
...
def process_winners():
    ...
    print(f"Found {len(winners)} winners to process\n")

    enhanced_results = {}

    for project_id, winner in list(winners.items())[:10]:  # Process first 10
    ...
    # Save results
    with open('data/winner_emails_enhanced.json', 'w') as f:
    ...
    print(f"Saved to data/winner_emails_enhanced.json")
